/*
  # Gaming Center Database Setup

  1. Initial Data Setup
    - Create roles with proper permissions
    - Set up equipment types and rate profiles
    - Create sample consoles, suppliers, products, and customers
  ...
  3. Sample Data
    ...
    - Sample consoles and gaming stations
    - Products for cafe/POS system
...
BEGIN
  ...
  INSERT INTO vouchers (
    ...
    created_date, expiry_date, status, customer_id, customer_name, customer_phone,
    sold_date, sold_by, is_active, created_by
  ) VALUES
    (
      ...
      customer1_id, 'Ahmad Rizki', '+628123456789',
      CURRENT_DATE, admin_user_id, true, admin_user_id
    ),
    (
      ...
      customer2_id, 'Budi Santoso', '+628345678901',
      CURRENT_DATE, admin_user_id, true, admin_user_id
    ),
    (
      ...
      NULL, NULL, NULL, CURRENT_DATE - INTERVAL '25 days', admin_user_id, true, admin_user_id
    )
  ON CONFLICT DO NOTHING;
END $$;

-- Create some sample rental sessions
DO $$
DECLARE
  customer1_id uuid;
  customer2_id uuid;
  admin_user_id uuid;
BEGIN
  -- Get some customer and user IDs
  SELECT id INTO customer1_id FROM customers WHERE name = 'Ahmad Rizki' LIMIT 1;
  ...
  INSERT INTO rental_sessions (
    ...
    applied_rate_profile, created_by
  ) VALUES
    (
...
BEGIN
  ...
  IF product1_id IS NOT NULL AND product2_id IS NOT NULL THEN
    ...
    INSERT INTO sales (
      ...
      payment_method, payment_amount, change_amount, sale_date, cashier_id
    ) VALUES (
      sale_id, customer1_id, 13000, 0, 0, 13000,
      'cash', 15000, 2000, CURRENT_TIMESTAMP, admin_user_id
    );
