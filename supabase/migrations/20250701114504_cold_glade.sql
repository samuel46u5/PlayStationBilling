/*
  ...
  1. New Tables
    - Creates initial roles with proper permissions
    - Sets up initial users in the users table
  ...
  3. Initial Data
    - Creates admin, manager, cashier, and staff roles
    - Sets up demo users with proper role assignments
    - Provides authentication-ready user accounts

  Note: This migration sets up the user management system with demo accounts.
  The actual Supabase Auth users will need to be created through the Supabase dashboard
  or using the Supabase CLI, as SQL migrations cannot directly create auth.users records.
*/
...
CREATE POLICY "Admins can read all users"
  ...
  USING (
    EXISTS (
      ...
      AND 'users.read' = ANY(r.permissions::text[])
    )
  );
...
CREATE POLICY "Admins can insert users"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE u.id = auth.uid() 
      AND r.name = 'Administrator'
      AND 'users.write' = ANY(r.permissions::text[])
    )
  );
