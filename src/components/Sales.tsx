...
const Sales: React.FC = () => {
  ...
  function getPeriodStartISO(period: string): string | null {
    const now = new Date();
  ...
  function mapTransactionRow(row: any): Sale {
    const details = row?.details || {};
    const itemsRaw: any[] = Array.isArray(details.items) ? details.items : [];
    ...
    const items: SaleItem[] = itemsRaw.map((it: any) => {
      ...
      return {
        productId: String(it?.name ?? "Item"),
        productName: String(it?.name ?? "Item"),
        quantity: qty,
        price,
        total,
      };
    ...
    // Tentukan total transaksi: prefer breakdown.products_total, fallback dari payment.amount - payment.change
    const productsTotal = toNumber(breakdown.products_total);
    const payAmount = toNumber(payment.amount);
    const payChange = toNumber(payment.change);
    const computedTotal =
      productsTotal > 0 ? productsTotal : Math.max(payAmount - payChange, 0);
    ...
    return {
      ...
      discount: 0,
      total: computedTotal,
      paymentMethod: String(payment.method || row.payment_method || "cash") as
        | "cash"
        | "card"
        | "transfer",
      paymentAmount: payAmount,
      changeAmount: payChange,
      saleDate: row.timestamp,
  ...
  async function fetchSales() {
    ...
    try {
      ...
      let query = supabase
        ...
        .select(
          "id, amount, payment_method, timestamp, cashier_id, type, reference_id, description, details"
        )
        .eq("type", "sale")
      ...
      const mapped = (data || []).map(mapTransactionRow);
      setSales(mapped);
      if (selectedSale && !mapped.find((s) => s.id === selectedSale)) {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {["cash", "card", "transfer"].map((method) => {
              ...
              const percentage =
                totalSales > 0 ? (methodTotal / totalSales) * 100 : 0;
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getPaymentMethodColor(
                        sale.paymentMethod
                      )}`}
                    >
                      ...
                      {sale.paymentMethod === "cash" && (
                        <>
                          <div className="flex justify-between">
                            ...
                            <span>
                              Rp {sale.paymentAmount.toLocaleString("id-ID")}
                            </span>
                          </div>
                          <div className="flex justify-between">
