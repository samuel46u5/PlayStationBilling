      ...
      {activeTab === "roles" && (
        <div className="py-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-900">Daftar Roles</h2>
            <button
              ...
              <table className="min-w-full bg-white rounded-xl shadow-sm border border-gray-200">
                <thead>
                  <tr className="bg-gray-100">
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nama Role</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Deskripsi</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Jumlah Permissions</th>
                ...
                <tbody>
                  {roles.map((role: any) => (
                    <tr key={role.id} className="border-t">
                      ...
                      <td className="px-6 py-4 text-gray-700">{role.permissions ? role.permissions.length : 0}</td>
...
const EditRoleModal: React.FC<EditRoleModalProps> = ({ role, onClose, onSaved }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const permissionsPayload = mockPermissions.filter((p) => permissions.includes(p.id));
      // update in db
      await db.update('roles', role.id, { name, description, permissions: permissionsPayload });
      const updated = { ...role, name, description, permissions: permissionsPayload };
      Swal.fire({ icon: 'success', title: 'Role diperbarui' });
      onSaved(updated);
    } catch (err) {
      const message = (err && (err as any).message) || 'Gagal menyimpan role';
      Swal.fire({ icon: 'error', title: 'Gagal menyimpan role', text: message });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="p-6">
          ...
          <form className="space-y-4" onSubmit={handleSave}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button type="submit" disabled={loading} className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg">{loading ? 'Menyimpan...' : 'Save'}</button>
            </div>
          </form>
          <div className="mt-6">
              ...
              {mockPermissions.map((perm) => (
                <label key={perm.id} className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) setPermissions((s) => [...s, perm.id]);
                      else setPermissions((s) => s.filter((id) => id !== perm.id));
                    }}
                  />
        ...
        <div className="p-6">
          ...
          <form className="space-y-4" onSubmit={handleCreate}>
            <div>
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="Enter role name" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="Describe the role" />
            </div>
            <div>
            ...
            <div className="flex gap-3 mt-6">
              <button type="button" onClick={onClose} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg">Cancel</button>
              <button type="submit" disabled={loading} className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg">{loading ? 'Membuat...' : 'Create Role'}</button>
            </div>
          </form>
        </div>
      </div>
